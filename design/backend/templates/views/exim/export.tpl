    ...
    {if $pattern.options}
        ...
        <div class="control-group">
            ...
            <div class="controls">
                ...
                {elseif $o.type == "range_date"}
                    {if $settings.Appearance.calendar_date_format == "month_first"}
                        {assign var="exim_date_format" value=$smarty.now|date_format:"%m/%d/%Y"}
                    {else}
                        {assign var="exim_date_format" value=$smarty.now|date_format:"%d/%m/%Y"}
                    {/if}

                    {include file="common/calendar.tpl" date_id="elm_from_date_{$p_id}" date_name="from_date" date_val="{$smarty.now|date_format:$exim_date_format}" start_year="1902" end_year="0" }
                    {include file="common/calendar.tpl" date_id="elm_to_date_{$p_id}" date_name="to_date" date_val="{$smarty.now|date_format:$exim_date_format}" start_year="1902" end_year="0" }
