...
{capture name="shipping_address"}
    ...
    {if !fn_is_empty($user_data)}
        {if $profile_fields.S}
            {assign var=self_location_shipping value=""}
            {foreach from=$user_data.shipping item="shipping" key="shipping_id" name="f_shipp"}
                {if $shipping.module == "self_collection"}
                    {$self_location_shipping=$shipping}
                    {break}
                {/if}
            {/foreach}

            {if !empty($self_location_shipping)}
                <p class="strong">Self Pickup: {$self_location_shipping.shipping}</p>
                <p>Outlet Name: {$self_location_shipping.service_params.outlet_name|default:'none'}</p>
                <p>Location: {$self_location_shipping.service_params.location|default:'none'}</p>
                <p>Address: {$self_location_shipping.service_params.address|default:'none'}</p>
            {else}
