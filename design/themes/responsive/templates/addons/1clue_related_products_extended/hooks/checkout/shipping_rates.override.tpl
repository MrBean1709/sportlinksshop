    ...
    <div id="shipping_rates_list">
        ...
        {foreach from=$product_groups key="group_key" item=group name="spg"}
            ...
            {if $group.shippings && !$group.all_edp_free_shipping && !$group.all_free_shipping && !$group.free_shipping && !$group.shipping_no_required}
                ...
                {* DEV *}
                {assign var="shippings_self_collection" value=array()}
                {assign var="shippings_aramex" value=array()}
                {foreach from=$group.shippings item="shipping"}
                    {if $shipping.module == "self_collection"}
                        {$shippings_self_collection[] = $shipping}
                    {else}
                        {$shippings_aramex[] = $shipping}
                    {/if}
                {/foreach}

                {if !empty($shippings_self_collection)}
                    <h3>Self Collection at Outlets</h3>
                    {foreach from=$shippings_self_collection item="shipping"}
                ...
                {if !empty($shippings_aramex)}
                    <h3>Local Delivery</h3>
                    {foreach from=$shippings_aramex item="shipping"}
