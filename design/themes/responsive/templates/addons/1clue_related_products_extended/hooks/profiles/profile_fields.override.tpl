<div class="ty-control-group ty-profile-field__item ty-{$field.class}">
    ...
    {else}  {* Simple input *}
        {if $data_id|strpos:"city" !== false}
        <input style="display:none" {if $field.autocomplete_type}x-autocompletetype="{$field.autocomplete_type}"{/if} type="text" id="{$id_prefix}elm_{$field.field_id}" name="{$data_name}[{$data_id}]" size="32" value="Singapore" class="ty-input-text {if !$skip_field}{$_class}{else}cm-skip-avail-switch{/if}" {if !$skip_field}{$disabled_param nofilter}{/if} />
        {else}
