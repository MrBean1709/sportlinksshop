...
if ($mode == 'export') {
    ...
    // dev: third party
    unset($patterns['sale_orders']); unset($patterns['sale_lines']); unset($patterns['tender_records']);
    // end dev
...
function fn_export($pattern, $export_fields, $options)
{
    ...
    // dev: third party
    if (!empty($pattern['pattern_id']) && !empty($_REQUEST['from_date']) && !empty($_REQUEST['to_date'])
        && ($pattern['pattern_id'] == 'sale_orders' || $pattern['pattern_id'] == 'sale_lines' || $pattern['pattern_id'] == 'tender_records') ) {
        $_from_time = fn_parse_date($_REQUEST['from_date']);
        $_to_time = fn_parse_date($_REQUEST['to_date'], true);

        $conditions[] = "orders.timestamp >= $_from_time AND orders.timestamp <= $_to_time";
    }
    // end dev
